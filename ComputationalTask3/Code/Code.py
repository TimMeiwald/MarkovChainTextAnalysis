...
@author: %(Tim Meiwald)s
"""

Text = """ Vor Zeiten war ein König und eine Königin, die sprachen jeden Tag: "Ach, wenn wir doch ein Kind hätten!" und kriegten immer keins.
...
def StripSymbols(Text):
    Symbols = ["!",":",";",",",".","  ","?","'",'"']
    N = 0
    #Counts number of appearances of Symbols then Strips Symbols
    for i in Symbols:
        N += Counter(Text,i)
        Text = Text.replace(i,"")
    return Text,N
...
def TextInformation(Text):
    Text_Stripped = StripSymbols(Text)
    NumberOfStrippedSymbols = Text_Stripped[1]
    Text = Text_Stripped[0].lower()
    #Removes whitespace except single spaces between words
    Text = " ".join(Text.split())
    Length = len(Text)
    Vowels = 0
    Consonants = 0
    for i in ["a","e","i","o","u","ä","ë","ö","y"]:
        Vowels += Counter(Text,i)
    for i in ["b","c","d","f","g","h","j","k","l","m","n","p","q","r","s","t","v","w","x","z","ß"]:
        Consonants += Counter(Text,i)
    NumberOfSpaces = Counter(Text," ")
    return Length,Vowels,Consonants,NumberOfSpaces,NumberOfStrippedSymbols,Text


def TransitionProbability(String):
    String = " ".join(String.split())
    String = StripSymbols(String)[0]
    String = String.lower()
    Alphabet = ["a","ä","b","c","d","e","ë","f","g","h","i","j","k","l","m","n","o","ö","p","q","r","s","ß","t","u","v","w","x","y","z"," "]
    ResultsArr = np.zeros((len(Alphabet),len(Alphabet)),dtype = "float64")
    ConfidenceIntervalsArr = np.zeros((len(Alphabet),len(Alphabet)),dtype = "float64")
    Counteri = -1
    Counterj = -1
    for i in Alphabet:
        Counteri +=1
        ...
        for j in Alphabet:
            ...
            CountFalse = 0
            Counterj += 1
            for k in np.arange(0,len(String)-1,1):
                if(i == String[k]):
                    if(j == String[k+1]):
                        CountTrue += 1
                    else:
                        CountFalse += 1
            if(CountFalse == 0):
                ResultsArr[Counteri,Counterj] = -0.1
                ConfidenceIntervalsArr[Counteri,Counterj]  = -0.1
            else:
                ResultsArr[Counteri,Counterj] = np.float64(CountTrue)/np.float64(CountTrue + CountFalse)
                ConfidenceIntervalsArr[Counteri,Counterj] = 1.96*np.sqrt((ResultsArr[Counteri,Counterj]*(1.-ResultsArr[Counteri,Counterj]))/np.float64(CountFalse))
    return ResultsArr,ConfidenceIntervalsArr

Result = TextInformation(Text)
Result2 = TransitionProbability(Text)
#np.savetxt("ConfidenceIntervals",Result2[1],delimiter =",")
import matplotlib.pyplot as plt
plt.subplot(121)
plt.title("Transition Probabilities")
plt.imshow(Result2[0],cmap = "inferno")
plt.colorbar(shrink = 0.6)

plt.subplot(122)
plt.title("Confidence Intervals")
plt.imshow(Result2[1],cmap = "inferno")
plt.colorbar(shrink = 0.6)
plt.close()
...
def VowelsConsonants(String):
    ...
    String = StripSymbols(String)[0]
    String = String.lower()
    Vowels = ["a","e","i","o","u","ä","ë","ö","y"]   
    Consonants = ["b","c","d","f","g","h","j","k","l","m","n","p","q","r","s","t","v","w","x","z","ß"]
    for i in Vowels:
        String = String.replace(i,"a")
    for i in Consonants:
        String = String.replace(i,"b")
    ResultsArr = np.zeros((3,3),dtype = "float64")
    ConfidenceIntervalsArr = np.zeros((3,3),dtype = "float64")
    Counteri = -1
    Counterj = -1
    for i in ["a","b"," "]:
        Counteri +=1
...
Result3 = VowelsConsonants(Text)
#np.savetxt("Task4Table1",Result3[0],delimiter =",")
#np.savetxt("Task4Table2",Result3[1],delimiter =",")


def GenerateString(N,SymbolList,ProbMat,StartingInt):
    ProbMat = np.asarray(ProbMat)
    #Dimensions of ProbabilityMatrix
    Shape = ProbMat.shape
    #Sum of each row from left to right for each value
    #First value is first probability matrix value
    #Second value is first + second matrix value
    # etc etc until the last value should be 1 since the sum of every character and space following
    # a specific character should be 1. 
    #Not perfect due to roundoff error in some cases. 
    ProbMatSum = ProbMat.copy()
    #Clears the negative 0.1 values assigned whhere a character didn't exist 
    #for easier visualization purposes returns to 0
    ProbMat[np.where(ProbMatSum < 0)] = 0
    for i in np.arange(0,Shape[0],1):
        for j in np.arange(0,Shape[1],1):
            ProbMatSum[i,j] = np.sum(ProbMat[i,:j+1])
    ResultsArr = np.zeros(N,dtype = "int16")
    #Start with Consonant since most probable
    #Vowels = 1,Consonants = 2, Spaces = 3
    ResultsArr[0] = StartingInt
    RandVal = np.random.rand(N)
    for i in np.arange(1,N,1):
        LastVal = ResultsArr[i-1]
        for j in np.arange(0,Shape[1],1):
            #Uniform distribution from 0 to 1
            if(RandVal[i] >= ProbMatSum[LastVal-1,j]):
                continue
            #This if just prevents the value updating if the new value has a 0 probability
            #because otherwise nonexistent characters will turn up as frequently as the previous
            #character which isn't ideal. 
            if(ProbMatSum[LastVal-1,j] == ProbMatSum[LastVal-1,j-1]):
                continue
            else:
                ResultsArr[i] = j+1
                break
    ResultsString = ""
    for i in ResultsArr:
        ResultsString += SymbolList[i-1]
    return ResultsString

Result4 = GenerateString(100,["a","b"," "],np.asarray(Result3[0]),2)
Result5 = GenerateString(1000,["a","ä","b","c","d","e","ë","f","g","h","i","j","k","l","m","n","o","ö","p","q","r","s","ß","t","u","v","w","x","y","z"," "],np.asarray(Result2[0]),6)